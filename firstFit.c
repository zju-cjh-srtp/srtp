...
int* UsePNodeInOrder(int taskNum, int pNodeNum, struct vList* list, int dis[][PN_MAX], struct pNode** pNodeList,FILE *fr)  //对每一个子任务，按物理机编号顺序找到第一个装得下的物理机，并放入。返回每个任务的完成时间
{
	int tmp_time = 0;
